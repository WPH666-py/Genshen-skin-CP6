# -*- coding: utf-8 -*-
"""
引擎同步工具 —— 让 CP1~CP6 共用同一套皮肤引擎, 不再各改一份。
...
def engine_dir(key):
    """引擎文件所在目录: CP1 是包根, CP2~CP6 是包内的 engine/。"""
    v = PACKS[key]
...
def _role_of(key):
    ...
    return {"CP2": "odette_voj", "CP3": "cp3_trio",
            "CP4": "cp4_pair", "CP5": "cp5_pair", "CP6": "cp6_trio"}.get(key)
...
def normalize(text, src_slug, dst_slug, dst_key=None, src_key=None, src_repo=None,
    ...
    还要改写「常量模块」的导入, 各套件结构不同:
      ...
      CP4: from ..characters import cp6_trio as C
    ...
    CN = {
        ...
        "CP3": ("原神 CP 壁纸套件 3", "原神CP3", "玛拉妮 × 基尼奇 × 卡其娜", "~/.genshen-cp3"),
        "CP4": ("原神 CP 壁纸套件 6", "原神CP6", "玛拉妮 × 基尼奇 × 卡其娜", "~/.genshen-cp6"),
    }
    ...
    # 注意: 源与目标的角色模块名可能不同(cp6_trio vs odette_voj),
...
def main():
    ap = argparse.ArgumentParser(
        description="CP1~CP6 皮肤引擎同步工具(本机存在哪些套件就处理哪些)")
