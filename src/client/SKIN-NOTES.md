...
本目录的 `genshen-cp6.module.css` 同时服务两个消费者，改它之前请读完本文。
...
`genshen-cp6.module.css` 的注释里绝对不能出现：
...
- 想保留夜景蓝紫的逐槽位校色版本，用：
  ...
  ```bash
  genshen-cp6 deepking          # 导出 ~/.genshen-cp6/deepking/*.skin.json
  ```
...
```bash
genshen-cp6 deepking --check    # 应输出「通过」
genshen-cp6 deepking --what     # 看转换器实际会取到什么值
```
...
- `mascot-cp6-light.jpg` —— 取「三人」那张的夕照合影, 配浅色界面
- `mascot-cp6-dark.jpg` —— 取「午后」那张的暖调三人, 配深色界面
