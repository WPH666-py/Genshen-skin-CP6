...
def pkg_root():
    """包目录 .../genshen_skin_cp6(本模块在它的 engine/ 下, 故向上一层)。
...
# src/client/genshen-cp6.module.css 完全一致的取值, 保证在线转换与离线生成一致。
...
FALLBACK_CSS = """:root {
  ...
  --bg-layer-2: #eaf3f7;
  --bg-layer-3: #d6e7ef;
  --label-primary: #14263a;
  --label-secondary: #3f556e;
  --label-tertiary: #71879c;
  --brand-primary: #2e9ec4;
  --accent: #2e9ec4;
  --primary: #2e9ec4;
  --interactive-bg-hover-solid: #ddeef5;
  --interactive-bg-active: #c4e1ec;
  --border-l1: #d4e6ee;
  --border-l2: #bcd8e4;
  --border: #bcd8e4;
}
        ...
        <section class="mock" style="background:%(bg)s;color:%(bgText)s">
          ...
          <div class="body">
            <aside style="background:%(sidebarBg)s;color:%(sidebarText)s;border-color:%(border)s">
              ...
              <div class="item">📁 Genshen-skin-CP6</div>
...
<h2>🌙 暗色 Dark</h2>%(dark)s
        ...
        "light": region(p, "亮色 · 原神CP6", mascot.get("light")),
        "dark": region(d, "暗色 · 原神CP6", mascot.get("dark")),
    }
