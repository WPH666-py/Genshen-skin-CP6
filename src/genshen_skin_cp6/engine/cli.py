...
本套件有两张素材(玛拉妮、基尼奇、卡其娜), 每张三种摆法, 共 6 种壁纸:

  genshen-cp6                 # 默认: single1 卡片式(模糊背景 + 居中圆角卡片)
  genshen-cp6 cover           # 满屏: cover 裁切铺满整屏, 无边框
  genshen-cp6 showall         # 完整: 等比放进纯色底, 一个像素都不裁
  genshen-cp6 1               # 等价于 single1(兼容按张编号的写法)
  genshen-cp6 list            # 列出所有可切换样式
  genshen-cp6 random          # 随机来一张
  genshen-cp6 all             # 生成全部样式到 ~/.genshen-cp6/wallpapers
  genshen-cp6 all --out DIR   # 生成到指定目录(JetBrains 背景图用)
  genshen-cp6 cycle 30        # 每 30 分钟自动随机换壁纸
  genshen-cp6 switcher        # 打开可视化切换器
  genshen-cp6 pet             # 启动桌面桌宠
  genshen-cp6 deepking        # 生成 DeepKing 界面皮肤 + 离线预览
  genshen-cp6 copy            # 只合成不设置
...
def build_parser():
    ...
    # 每一种模式都注册成子命令。
    # argparse 的子命令是"贪婪"的: 一旦注册了子解析器, `genshen-cp6 cover`
    ...
    # 覆盖掉子解析器 set_defaults 设的 mode, 于是 `genshen-cp6 cover2` 反而
