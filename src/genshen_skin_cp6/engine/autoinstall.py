...
原神 CP 壁纸套件 6 —— 自动安装器
  ...
  genshen-cp6-install                 # 自动: 装依赖 -> 生成壁纸 -> 设为桌面 -> 注册已装 IDE
  genshen-cp6-install --no-wallpaper  # 只做 IDE 注册, 不动桌面壁纸
  genshen-cp6-install --only vscode   # 只处理指定目标
...
def mcp_stdio_entry():
    ...
    return {
        ...
        "args": ["-m", "genshen_skin_cp6.mcp_server"],
        "env": {},
    }
