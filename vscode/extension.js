// 原神 CP 壁纸套件 6 — 玛拉妮 × 基尼奇 × 卡其娜 · 单张素材三种呈现 (VSCode / Trae / CodeX / Cursor / Windsurf)
...
const REPO_URL = 'https://github.com/WPH666-py/Genshen-skin-CP6.git';
const EXT_ID = 'wp666.genshen-skin-cp6';
...
function detectPython() {
  const cfg = expand(vscode.workspace.getConfiguration('genshencp6').get('pythonPath', ''));
...
function findRepo() {
  const cfg = expand(vscode.workspace.getConfiguration('genshencp6').get('repoPath', ''));
  const cands = [
    cfg,
    process.env.GENSHIN_CP1_REPO,
    path.join(os.homedir(), 'Genshen-skin-CP6'),
    process.env.USERPROFILE ? path.join(process.env.USERPROFILE, 'Genshen-skin-CP6') : null,
  ...
  for (const c of cands) {
    try {
      if (fs.existsSync(path.join(c, 'src', 'genshen_skin_cp6', 'cli.py'))) return c;
...
function buildInvocation(sub) {
  ...
  if (repo) {
    env.PYTHONPATH = (env.PYTHONPATH ? env.PYTHONPATH + path.delimiter : '') +
      path.join(repo, 'src');
    return { py: py, args: ['-m', 'genshen_skin_cp6.cli'].concat(sub), cwd: repo, env: env };
  }
  // pip 安装形态: 直接用模块
  return { py: py, args: ['-m', 'genshen_skin_cp6.cli'].concat(sub), cwd: os.homedir(), env: env };
}

function envError(msg) {
  vscode.window.showWarningMessage(
    msg + ' 请先安装: pip install ' + 'genshen-skin-cp6' +
    '  (或克隆 ' + REPO_URL + ' 到 ' + path.join(os.homedir(), 'Genshen-skin-CP6') + ')'
  );
}
...
function spawnGui(moduleName, label) {
  ...
  const child = spawn(exe, ['-m', 'genshen_skin_cp6.' + moduleName], {
    cwd: inv.cwd,
    env: inv.env,
    detached: true,
    stdio: 'ignore',
...
// 本套件只有一张素材(玛拉妮 × 基尼奇 × 卡其娜 双人合影), 因此画廊给的是同一张的
// 三种呈现方式。ids 必须与 characters/cp6_trio.py 的 MODES 一致。
const CARDS = [
  { mode: 'single1', t: '午后 · 卡片', img: 'thumb-single1.png', d: '模糊背景 + 居中卡片' },
  { mode: 'single2', t: '三人 · 卡片', img: 'thumb-single2.png', d: '模糊背景 + 居中卡片' },
  { mode: 'cover1', t: '午后 · 满屏', img: 'thumb-cover1.png', d: 'cover 铺满整屏' },
  { mode: 'cover2', t: '三人 · 满屏', img: 'thumb-cover2.png', d: 'cover 铺满整屏' },
  { mode: 'showall1', t: '午后 · 完整', img: 'thumb-showall1.png', d: '不裁切, 两侧留边' },
  { mode: 'showall2', t: '三人 · 完整', img: 'thumb-showall2.png', d: '不裁切, 两侧留边' }
];
...
function renderHtml(thumb, hasPython) {
  ...
  return '<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><style>' +
    ...
    '</div>' +
    '<div class="foot">原神 CP 壁纸套件 6 · 玛拉妮 × 基尼奇 × 卡其娜 v0.1.0<br/>' +
...
function activate(ctx) {
  ...
  reg('genshencp6.set1', () => runCli(['1'], '已切换: 午后'));
  reg('genshencp6.set2', () => runCli(['2'], '已切换: 三人'));
  reg('genshencp6.setCard', () => runCli(['card'], '已切换: 卡片式'));
  reg('genshencp6.setCover', () => runCli(['cover'], '已切换: 满屏'));
  reg('genshencp6.setShowall', () => runCli(['showall'], '已切换: 完整不裁'));
  reg('genshencp6.setRandom', () => runCli(['random'], '已随机换一张!'));
  reg('genshencp6.generateAll', () => runCli(['all'], '已生成全部样式!'));
  reg('genshencp6.openSwitcher', () => spawnGui('switcher', '壁纸切换器'));
  reg('genshencp6.startPet', () => spawnGui('pet', '桌面桌宠'));
  reg('genshencp6.showInfo', () => {
    const inv = buildInvocation(['info']);
    if (inv.error) return envError(inv.error);
    const term = vscode.window.createTerminal({ name: '原神CP6 自检', cwd: inv.cwd, env: inv.env });
    term.show();
    term.sendText('"' + inv.py + '" ' + inv.args.map((a) => '"' + a + '"').join(' '));
  });
  reg('genshencp6.openGallery', () =>
    vscode.commands.executeCommand('workbench.view.extension.genshencp6')
  );

  ctx.subscriptions.push(
    vscode.window.registerWebviewViewProvider('genshencp6.gallery', new GalleryProvider(), {
